...
if (!empty($_GET['tb'])) {
	...
	$query = $db->prepare("SELECT * FROM $tabla 
		INNER JOIN producto ON producto_despacho_idProducto=idProducto 
		INNER JOIN despacho ON producto_despacho_idDespacho=idDespacho 
		WHERE producto_despacho_idDespacho=:id AND despacho_estado='activo'");
    ...
    $pdf->SetTextColor(97, 97, 97);
    $pdf->Cell(45, 10, utf8_decode(' N.Consecutivo: ' . $data[0]['despacho_consecutivo']), 'L', 0, 'C');
	...
	foreach ($data as $row) {
		...
		$total = $row['producto_peso'] * $row['producto_despacho_cantidad'];

		$total_f += $total;
		// $total_f = $total + ($row['producto_peso'] * $row['producto_despacho_cantidad']);
		...
		$pdf->Cell(18, 10, utf8_decode($row['producto_despacho_cantidad']), 'L', 0, 'C');
	...
	$pdf->Cell(25, 10, utf8_decode('Datos del cliente final'), 0, 0);
	...
	$pdf->Cell(88, 8, utf8_decode($data[0]['despacho_cedulaPersona']), 0, 0);
	$pdf->SetTextColor(33, 33, 33);

	$pdf->Cell(15, 8, utf8_decode('Cédula:'), 0, 0);
	$pdf->SetTextColor(97, 97, 97);

	$pdf->Cell(40, 8, utf8_decode($data[0]['despacho_clienteF']), 0, 0);
	$pdf->SetTextColor(33, 33, 33);
	...
	$pdf->Cell(85, 8, utf8_decode($data[0]['despacho_nombrePersona']), 0, 0);
	$pdf->SetTextColor(33, 33, 33);

	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell(18, 8, utf8_decode('Codigo:'), 0, 0);

	$pdf->SetTextColor(97, 97, 97);

	$pdf->Cell(70, 8, utf8_decode($data[0]['despacho_codigo']), 0, 0);
	$pdf->SetTextColor(33, 33, 33);
	...
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell(70, 8, utf8_decode($data[0]['despacho_placaPersona']), 0, 0);
	$pdf->SetTextColor(33, 33, 33);

	$pdf->Ln(10);
	$pdf->Cell(103, 10, utf8_decode('Firma del conductor'), 0, 0);
	$pdf->Image('../img/signature/' . $dataImg[0]['imagen_nombre'], 10, 194, 90, 20, 'PNG');
